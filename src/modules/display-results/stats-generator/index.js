...
const StatsGenerator = ({appStateHook,setStatistics}) => {
  ...
  return <button className={classMaker(status,bufferLength)}
                 onClick={(e)=>onClick(e,appStateHook,workerInctance.current)}>
                  <span className="button-image cogs-image"></span><span className="button-title">Wygeneruj statystyki</span>
         </button>;
};
...
const onClick = (e,{setAppState,appState}, workerInctance)=> {
  e.target.blur();
